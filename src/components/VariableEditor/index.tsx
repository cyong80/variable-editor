...
/** @deprecated Use VariableEditorRootProps */
export type VariableEditorProps = VariableEditorRootProps;
...
const variableEditorRootVariants = cva("relative", {
  variants: {
    viewMode: {
      true: "variable-editor-view-mode",
      false: "",
    },
  },
  defaultVariants: {
...
const variableEditorInputVariants = cva(
  "variable-editor-input min-h-[140px] max-h-[320px] w-full overflow-y-auto rounded-xl border border-input/80 bg-background px-4 py-3.5 text-sm outline-none transition-shadow prose prose-sm max-w-none leading-relaxed focus:border-primary/40 focus:ring-2 focus:ring-primary/20",
  {
    variants: {
      disabled: {
...
const variableEditorFooterVariants = cva(
  "variable-editor-footer mt-3 flex flex-wrap items-center gap-3 rounded-lg border border-transparent bg-muted/30 px-1 py-2",
);

const variableEditorStatsVariants = cva(
  "variable-editor-stats text-xs tabular-nums text-muted-foreground",
  {
    variants: {
      overflow: {
...
>(function VariableEditorRoot(
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        ...(id && { id }),
        class: variableEditorInputVariants({ disabled }),
      },
    },
  });
  ...
  return (
    <VariableEditorContext.Provider value={contextValue}>
      <div
        ref={containerRef}
        className={cn(variableEditorRootVariants({ viewMode }), className)}
...
interface VariableEditorContentProps {
  className?: string;
}

function VariableEditorContent({ className }: VariableEditorContentProps) {
...
function VariableEditorFooter({ className, children }: VariableEditorFooterProps) {
  return (
    <div className={cn(variableEditorFooterVariants(), className)}>
      {children}
    </div>
  );
}
...
function VariableEditorStats({ className }: VariableEditorStatsProps) {
  ...
  return (
    <span
      className={cn(
        variableEditorStatsVariants({ overflow }),
...
export const VariableEditor: VariableEditorCompound = Object.assign(
  VariableEditorRoot,
  {
